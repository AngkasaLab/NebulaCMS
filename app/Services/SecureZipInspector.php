...
class SecureZipInspector
{
    ...
    /**
     * Enforce allowed extensions for theme ZIP uploads.
     *
     * @throws InvalidArgumentException
     */
    public function assertThemeZipExtensions(string $zipPath): void
    {
        $zip = new ZipArchive;
        if ($zip->open($zipPath) !== true) {
            throw new InvalidArgumentException('Cannot open ZIP archive.');
        ...
        $allowed = array_map('strtolower', config('upload_security.theme_zip_allowed_extensions', []));
        $allowedNames = array_map('strtolower', config('upload_security.theme_zip_allowed_extensionless_names', []));
